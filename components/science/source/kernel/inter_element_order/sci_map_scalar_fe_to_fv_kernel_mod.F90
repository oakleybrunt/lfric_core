...
!> @brief   The weighted mapping from a higher to a lower-order scalar space.
!> @details Take a scalar space which is higher-order in the horizontal but
!!          shares the same number of levels, and map it to a lowest-order
!!          scalar space on a finer mesh.

module sci_map_scalar_fe_to_fv_kernel_mod
...
type, public, extends(kernel_type) :: map_scalar_fe_to_fv_kernel_type
   private
...
contains
  procedure, nopass :: map_scalar_fe_to_fv_code
end type map_scalar_fe_to_fv_kernel_type

public :: map_scalar_fe_to_fv_code
...
contains
  ...
  subroutine map_scalar_fe_to_fv_code(nlayers,                 &
                                      cell_map,                &
        ...
        do k = 0, top_layer
          do df = 1, top_df
            fine_field(map_fine(df_f,cell_map(x_idx,y_idx))+k) = &
                fine_field(map_fine(df_f,cell_map(x_idx,y_idx))+k) &
              + coarse_field(map_coarse(df)+k)*weights(map_weights(df) + j)
          end do
        end do
      end do
    end do

  end subroutine map_scalar_fe_to_fv_code

end module sci_map_scalar_fe_to_fv_kernel_mod
